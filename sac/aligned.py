...
from apted import helpers, APTED
...
class AlignedTrees(SAC):
    ...
    def default_label_changes(self):
        ...
        if self._default_label_changes is None:
            self._default_label_changes = self.directional_n_changes(self.src2tgtlist_d,
                                                                     self.src_labels_map,
                                                                     self.tgt_labels_map)
    ...
    def avg_token_label_changes(self):
        if self._avg_token_label_changes is None:
            ...
            self._avg_token_label_changes = {
                'src': self.directional_n_changes(self.src2tgtlist_d,
                                                  self.src_labels_map,
                                                  self.tgt_labels_map,
                                                  method='token_avg'),
                'tgt': self.directional_n_changes(self.tgt2srclist_d,
                                                  self.tgt_labels_map,
                                                  self.src_labels_map,
                                                  method='token_avg')
            }
            self._avg_token_label_changes['avg'] = (self._avg_token_label_changes['src'] +
                                                    self._avg_token_label_changes['tgt']) / 2
    ...
    def overlap_label_changes(self):
        ...
        if self._overlap_label_changes is None:
            ...
            self._overlap_label_changes['avg'] = (self._overlap_label_changes['src'] +
                                                  self._overlap_label_changes['tgt']) / 2
    ...
    def src_labels_map(self):
        if self._src_labels_map is None:
            self._src_labels_map = {idx: f"src_{tree.label()}" for idx, tree in
                                    self.src_tree.word_order_idx_mapping.items()}
    ...
    def tgt_labels_map(self):
        if self._tgt_labels_map is None:
            self._tgt_labels_map = {idx: f"tgt_{tree.label()}" for idx, tree in
                                    self.tgt_tree.word_order_idx_mapping.items()}
    ...
    def merge_maps(self):
        ...
        for group in connected_groups:
            ...
            group_mapping = {item: mod_src2tgtlist_d[item] for item in group if item.startswith('src_')}
            group_mapping = {int(src[4:]): [int(tgt[4:]) for tgt in v] for src, v in group_mapping.items()}
            group_mapping = {
                self.src_labels_map[src_idx]: [self.tgt_labels_map[tgt_idx] for tgt_idx in v if tgt_idx != -1] for
                src_idx, v in group_mapping.items() if src_idx != -1}
