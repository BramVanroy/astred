...
def main(src, tgt, lang='en', pretokenized=False, draw=False):
    tree_ref = GenericTree.from_string(src, lang_or_model=lang, tokenize_pretokenized=pretokenized)
    tree_hyp = GenericTree.from_string(tgt, lang_or_model=lang, tokenize_pretokenized=pretokenized)
...
if __name__ == '__main__':
    ...
    cparser.add_argument('-t', '--pretokenized', help='Whether the text is already tokenized', action='store_true')
    cparser.add_argument('-d', '--draw', help='Whether to draw the trees (NOT recommended when running code remotely)',
                         action='store_true')
